...
export const UserDataProvider = ({ children }) => {
  ...
  // Determine if we're on a user page and extract username/section
  useEffect(() => {
    const pathParts = location.pathname.split('/').filter(part => part);
    
    if (pathParts.length === 0) {
      // Root path - show default data
      setCurrentUsername(null);
      setCurrentSection('home');
    } else if (pathParts.length === 1 && !['login', 'register', 'dashboard', 'share', 'wedding'].includes(pathParts[0])) {
      // Username path like /johndoe
      setCurrentUsername(pathParts[0]);
      setCurrentSection('home');
    } else if (pathParts.length === 2 && !['login', 'register', 'dashboard', 'share', 'wedding'].includes(pathParts[0])) {
      // Username + section path like /johndoe/gallery
      setCurrentUsername(pathParts[0]);
      setCurrentSection(pathParts[1]);
    } else {
      // Default routes like /rsvp, /gallery, etc.
      setCurrentUsername(null);
  ...
  // Load wedding data based on current route
  useEffect(() => {
      ...
      try {
        ...
        if (currentUsername) {
          ...
          if (currentSection !== 'home') {
            endpoint = `/api/wedding/user/${currentUsername}/${currentSection}`;
          } else {
            endpoint = `/api/wedding/user/${currentUsername}`;
          }
        } else {
          // Default data - return the default structure from backend
          setWeddingData(getDefaultWeddingData());
          setIsLoading(false);
          return;
        }
        
        console.log('Loading wedding data from:', endpoint);
        ...
        if (response.ok) {
          ...
          console.error('Failed to load wedding data:', response.status);
          setWeddingData(getDefaultWeddingData());
        }
      } catch (error) {
    ...
    loadWeddingData();
  }, [currentUsername, currentSection]);
  ...
  // Save wedding data to MongoDB backend
  const saveWeddingData = async (newData) => {
    ...
    if (isAuthenticated && userInfo?.sessionId) {
      try {
        ...
        // First try to UPDATE existing wedding data
        let response = await fetch(`${backendUrl}/api/wedding`, {
          method: 'PUT',
          headers: {
        ...
        // If update fails because wedding doesn't exist (404), create it first
        if (!response.ok && response.status === 404) {
          console.log('Wedding data not found, creating new wedding data...');
          
          // Create new wedding data
          response = await fetch(`${backendUrl}/api/wedding`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  const updateWeddingData = async (field, value) => {
    const updatedData = { ...weddingData, [field]: value };
    return await saveWeddingData(updatedData);
  };
  ...
  // Get the wedding data URL for sharing
  const getWeddingUrl = () => {
    if (isAuthenticated && userInfo?.username) {
      return `${window.location.origin}/${userInfo.username}`;
    }
    return window.location.origin;
  };

  // Get section-specific URL
  const getSectionUrl = (section) => {
    if (currentUsername) {
      return `${window.location.origin}/${currentUsername}/${section}`;
    }
    return `${window.location.origin}/${section}`;
  };
  ...
  const value = {
    ...
    // Wedding data
    weddingData: weddingData || getDefaultWeddingData(),
    
    // Current page context
    currentUsername,
    currentSection,
    isUserPage: !!currentUsername,
...
// Default wedding data function
function getDefaultWeddingData() {
  return {
    // Home section data
    couple_name_1: 'Sarah',
